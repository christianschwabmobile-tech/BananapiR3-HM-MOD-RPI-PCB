...
static long eq3loop_create_slave_dev( struct file *filp, const char* name )
{
	...
	/*
	 * A channel is reusable only after both sides are gone.
	 *
	 * created == 0 only means that the master has closed.  A slave
	 * (e.g. hmlangw) may still have this channel open and therefore
	 * still references its semaphore, wait queues and buffers.
	 */
	while (channel_index < EQ3LOOP_NUMBER_OF_CHANNELS &&
	       (control_data->channels[channel_index].created ||
		control_data->channels[channel_index].slave_open_count))
	{
		channel_index++;
	}
	...
	channel->devnode = MKDEV(MAJOR(control_data->devnode),
				   MINOR(control_data->devnode) + channel_index + 1);
	strncpy(channel->name, name, sizeof(channel->name) - 1);

	/*
	 * Initialize everything which open()/poll() can touch before the
	 * device becomes visible through device_create().
	 */
	...
	/*
	 * Mark the channel active before publishing the device.  Otherwise
	 * userspace could open the freshly created device while created is
	 * still zero.
	 */
	channel->created = 1;

	{
		...
		dev = device_create(control_data->class, NULL,
				    channel->devnode, NULL, "%s", name);
		if (IS_ERR(dev))
		{
			ret = PTR_ERR(dev);
			channel->created = 0;
			printk(KERN_ERR EQ3LOOP_DRIVER_NAME
			       ": failed to create slave %s: %ld\n",
			       name, ret);
			kfree(conn);
			goto out;
		}
